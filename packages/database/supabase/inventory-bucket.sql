...
drop policy if exists "Authenticated users can upload inventory" on storage.objects;
create policy "Authenticated users can upload inventory"
on storage.objects for insert
to authenticated
with check (bucket_id = 'inventory');

drop policy if exists "Authenticated users can update inventory" on storage.objects;
